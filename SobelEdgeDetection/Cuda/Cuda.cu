...
// stream processor
__global__ void sobel(float* r, float* h,  int width ,int height)
{
  ...
  int istart, iend, jstart, jend, yystart, xxstart, i, j;
  int sum1=0, sum2=0;
  ...
  i = a%width;
      ...
      iend = i + 4;
      iend = iend>=width?width-1:iend;

      jstart = j - 4;
      jstart = jstart<0?0:jstart;
      yystart = jstart<0?-jstart:0;
      jend = j + 4;
      jend = jend>=height?height-1:jend;
      // if(i == 5 && j == 5)printf("istart = %d, iend = %d, jstart = %d, jend = %d\n", istart,iend,jstart,jend);
        for(int y=jstart, yy=yystart;y<=jend;y++,yy++){
            for(int x=istart, xx=xxstart;x<=iend;x++,xx++){
              sum1 += r[(y*width)+x] * matrix[xx][yy];
              sum2 += r[(y*width)+x] * matrix[yy][xx];
          }
      }
      //printf("%d\n",count);
      //count++;

    h[a] = abs(sum1) + abs(sum2);
  }
}
...
int main(int argc, char* argv[]){
  ...
  ff.readImage(h_input, imagewidth*imageheight);
  ...
  int blocksPerGrid = ((imagewidth*imageheight) + threadsPerBlock - 1) / threadsPerBlock;
  sobel<<<blocksPerGrid, threadsPerBlock>>>(d_input, d_output, imagewidth, imageheight);
  ...
  write_to_fits(h_output,imageheight, imagewidth, Oname);
